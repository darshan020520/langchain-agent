...
import aiohttp
# used for customising our own serpAPi output format
...
# initialises llm with streaming enabled
...
# system prompt setup and chaining it to Human prompt and chat_history and agent_scratchpad
...
#Lets start setting up async tools now

# SerpAPI result wrapper
...
# we use custom serpapi tool to format the search on the map
@tool
...
# the declaration of function with base asynccallbackhandler makes llm recognise the members of the function and invoke them during token streaming

class QueueCallbackHandler(AsyncCallbackHandler):
    def __init__(self, queue: asyncio.Queue):
        self.queue = queue
        self.final_answer_seen = False  # controls stopping signal
    ...
    # Gets called automatically on every new token generated by the LLM
    ...
    # Called when the LLM finishes generating for the current step
